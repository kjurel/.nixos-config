    ...
    settings = {
      bar =
        let
          formatIcon =
            bg: fg: icon:
            "<span line_height='1.1' background='${bg}' foreground='${fg}'> ${icon} </span>";
          formatText =
            bg: fg: text:
            "<span line_height='1.1' background='${bg}' foreground='${fg}'> ${text} </span>";
        in
        {
          exclusive = true;
          fixed-center = true;
          ...
          clock = {
            format =
              formatIcon xcolors.blue1 xcolors.bg0 ""
              + formatText xcolors.blue0 xcolors.fg0 "{:%a %b %d - %I:%M %p}";
            format-alt =
              formatIcon xcolors.blue1 xcolors.bg0 "" + formatText xcolors.blue0 xcolors.fg0 "{:%H:%M}";
            tooltip-format = "<big>{:%Y %B}</big>\n<tt><small>{calendar}</small></tt>";
          };

          cpu = {
            format =
              formatIcon xcolors.yellow1 xcolors.bg0 "" + formatText xcolors.bg1 xcolors.yellow0 "{usage}%";
          };
          ...
          "custom/playerctl" =
            ...
            in
            {
              ...
              format =
                formatIcon xcolors.purple1 xcolors.bg0 "{icon}" + formatText xcolors.bg1 xcolors.purple0 "{}";
              format-icons = {
                "Paused" = "";
                "Playing" = "󰎈";
              };
